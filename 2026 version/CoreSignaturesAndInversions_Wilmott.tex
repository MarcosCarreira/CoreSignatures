...
\begin{figure}[H]
\centering
\includegraphics[scale=0.45]{figures/FitSint1l2}\hfill\includegraphics[scale=0.45]{figures/FitSint2l2}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.45]{figures/Fit4pPath1}\hfill\includegraphics[scale=0.45]{figures/Fit4pPath2}
\caption{Fits of 4 and 5 segments to $\{t,\sin(k\pi t)\}$.\label{fig:sinpaths4pts}}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[scale=0.4]{figures/SemiCircle3}\hfill\includegraphics[scale=0.4]{figures/Digit5_5}
...
\paragraph{Mean-$L^{1}$ as an evaluation protocol.}
The role of the mean-$L^{1}$ evolutionary algorithm here is not that
of a second inversion algorithm meant to coincide numerically with
the algebraic Chen root: it is an \emph{independent test of the
inversion in path space}. We propose this as a clean evaluation protocol
for any signature-based inversion: pick a path-space distance with a
clear geometric meaning, minimise it directly by an algorithm that
knows nothing about signatures, and use the resulting approximant as
a benchmark against which the signature-based inversion is assessed.
Calibrating the regression weights $w_{w}$ to minimise the resulting
gap against this benchmark, on a training set spanning rough and
smooth paths, is the empirical complement to a proof of Conjecture
